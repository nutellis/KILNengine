...
constexpr auto erase_feature_struct_member(vk::Bool32& inout, const vk::Bool32 in) -> void
{
    if (in)
    {
        inout = vk::False;
...
{
    erase_feature_struct_member(inout.shaderIntegerDotProduct, in.shaderIntegerDotProduct);
}
...
{
    erase_feature_struct_member(inout.pipelineProtectedAccess, in.pipelineProtectedAccess);
}
