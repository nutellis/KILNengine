...
#ifdef KILN_TEMP_INVOKE_QUALS
  #error KILN_TEMP_INVOKE_QUALS should not be defined
#endif
#ifdef KILN_TEMP_CONST_REF
  #error KILN_TEMP_CONST_REF should not be defined
#endif

#ifndef KILN_TEMP_CONST
  #define KILN_TEMP_CONST
#endif
#ifndef KILN_TEMP_REF
  #define KILN_TEMP_REF
  #define KILN_TEMP_INVOKE_QUALS KILN_TEMP_CONST&
#else
  #define KILN_TEMP_INVOKE_QUALS KILN_TEMP_CONST KILN_TEMP_REF
#endif

#define KILN_TEMP_CONST_REF KILN_TEMP_CONST KILN_TEMP_REF
...
class Invoker {
    ...
    struct Dispatch {
        constexpr static auto operator()(
            KILN_TEMP_CONST internal::Storage<size_T, alignment_T>& storage,
            Args_T... args
        ) noexcept(KILN_TEMP_IS_NOEXCEPT) -> Result_T
        {
            if constexpr (uses_small_buffer_optimization_c<T, size_T, alignment_T>)
            {
                return std::invoke_r<Result_T>(
                    static_cast<T KILN_TEMP_INVOKE_QUALS>(*storage.template launder<T>()),
            ...
            else
            {
                ...
                return std::invoke_r<Result_T>(
                    static_cast<T KILN_TEMP_INVOKE_QUALS>(*handle),
                    std::forward<Args_T>(args)...
                );
            }
        }
...
public:
    ...
    constexpr auto operator()(
        KILN_TEMP_CONST internal::Storage<size_T, alignment_T>& storage,
        Args_T... args
    ) KILN_TEMP_CONST noexcept(KILN_TEMP_IS_NOEXCEPT) -> Result_T
    {
...
private:
    using InvokeFunc
        = auto(KILN_TEMP_CONST internal::Storage<size_T, alignment_T>&, Args_T...)
            -> Result_T;
...
{
public:
    using Signature = auto(FArgs_T...) KILN_TEMP_CONST_REF noexcept(KILN_TEMP_IS_NOEXCEPT)
        -> Result_T;
    ...
    constexpr auto operator()(FArgs_T... args) KILN_TEMP_CONST_REF noexcept(
        KILN_TEMP_IS_NOEXCEPT
    ) -> Result_T;
...
private:
    ...
    KILN_TEMP_NAMESPACE::Invoker<size_T, alignment_T, Result_T, FArgs_T...> m_invoke;
...
constexpr auto Function<
    ...
    size_T,
    alignment_T>::
    operator()(FArgs_T... args) KILN_TEMP_CONST_REF noexcept(KILN_TEMP_IS_NOEXCEPT)
        -> Result_T
{
...
constexpr bool internal::callable_with_qualifiers_v<
    T,
    auto(Args_T...) KILN_TEMP_CONST_REF noexcept(KILN_TEMP_IS_NOEXCEPT)->Result_T>
    = KILN_TEMP_NAMESPACE::callable_with_qualifiers_v<T, Result_T, Args_T...>;

#undef KILN_TEMP_NAMESPACE
#undef KILN_TEMP_CONST
#undef KILN_TEMP_REF
#undef KILN_TEMP_IS_NOEXCEPT
#undef KILN_TEMP_INVOKE_QUALS
#undef KILN_TEMP_CONST_REF
